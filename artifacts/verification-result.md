...
- `punch-list.json` when the run is blocked by missing evidence or human judgment.
...
2. `certificate_issued` is only valid when the proof summary has zero `judgmentRequired` and zero `evidenceMissing`.
